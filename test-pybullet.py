...
class RobotSim:
    ...
    def _put_input_to_sim(self, u: np.ndarray, control_type):
        """로봇을 원하는 제어 입력과 control type에 맞게 제어한다.

        Args:
            u (np.ndarray): 제어 입력 벡터
            control_type: 제어 타입 (예: p.POSITION_CONTROL, p.VELOCITY_CONTROL, p.TORQUE_CONTROL)
        """
        # 예: 특정 조인트(관절) 위치 제어
        # Panda 로봇의 조인트 인덱스는 URDF 구조에 따라 다름
        for j in range(self.joint_number):
            p.setJointMotorControl2(self.robotId, j, control_type, targetPosition=u[j])
    ...
    def control(self, state) -> np.ndarray:
        ...
        np_array = np.zeros(self.joint_number)
        ...
        while t <= self.simulation_duration:
            ...
            for _ in range(delay_steps):
                if t > self.simulation_duration:
                    break
                self._put_input_to_sim(u, control_type=p.POSITION_CONTROL)
                p.stepSimulation()
                current_pos = [
                    state[0]
            ...
            for _ in range(int(self.ctrl_dt / self.dt) - delay_steps):
                if t > self.simulation_duration:
                    break
                self._put_input_to_sim(u_new, control_type=p.POSITION_CONTROL)
                p.stepSimulation()
                current_pos = [
                    state[0]
                    for state in p.getJointStates(
    ...
    def save_simulation_data(self):
        ...
        if not os.path.exists("sim_data"):
            ...
            with open("sim_data/.gitignore", "w") as f:
                f.write("*")
        np.save("sim_data/log_traj.npy", np.array(self.log_traj))

    def visualize(self, fps: int = 60):
...
if __name__ == "__main__":
    ...
    sim.simulate()
    sim.save_simulation_data()
    print("시뮬레이션 데이터 저장 완료.")
    sim.visualize(fps=50)
    print("시뮬레이션 재생 완료.")
